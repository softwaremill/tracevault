<script lang="ts">
	...
	async function loadSettings() {
		loading = true;
		try {
			settings = await api.get<ComplianceSettings>(`/api/v1/orgs/${orgId}/compliance`);
	...
	async function handleSave(e: Event) {
		...
		success = '';
		try {
			settings = await api.put<ComplianceSettings>(`/api/v1/orgs/${orgId}/compliance`, {
					...
					onclick={async () => {
						try {
							const res = await api.get<{ algorithm: string; public_key: string }>(
								`/api/v1/orgs/${orgId}/compliance/public-key`
							);
