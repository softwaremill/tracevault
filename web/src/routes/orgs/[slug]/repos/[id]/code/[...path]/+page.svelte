	...
	async function loadContent(path: string, urlRef: string | null) {
		...
		try {
			if (branches.length === 0) {
				branches = await api.get<BranchInfo[]>(`/api/v1/orgs/${slug}/repos/${repoId}/code/branches`);
			}
			...
			try {
				treeEntries = await api.get<TreeEntry[]>(
					`/api/v1/orgs/${slug}/repos/${repoId}/code/tree?ref=${encodeURIComponent(resolvedRef)}&path=${encodeURIComponent(path)}`
				);
				isDirectory = true;
			} catch {
				blob = await api.get<BlobResponse>(
					`/api/v1/orgs/${slug}/repos/${repoId}/code/blob?ref=${encodeURIComponent(resolvedRef)}&path=${encodeURIComponent(path)}`
				);
				isDirectory = false;
			}
		} catch (err) {
...
<div class="space-y-4">
	<div class="flex items-center gap-2">
		...
		<a href="/orgs/{slug}/repos/{repoId}/code?ref={encodeURIComponent(gitRef)}" class="text-muted-foreground hover:underline">Code</a>
		<span class="text-muted-foreground">/</span>
