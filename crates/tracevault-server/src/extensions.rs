...
impl SigningProvider for CommunitySigningProvider {
    fn record_hash(&self, _canonical_json: &[u8]) -> String { String::new() }
    fn chain_hash(&self, _prev: Option<&str>, _record_hash: &str) -> String { String::new() }
    fn sign(&self, _record_hash: &str) -> String { String::new() }
    fn verify(&self, _record_hash: &str, _signature_b64: &str) -> bool { false }
    fn public_key_b64(&self) -> String { String::new() }
...
impl StoryProvider for CommunityStoryProvider {
    ...
    fn provider_name(&self) -> &str { "none" }
...
impl PricingProvider for CommunityPricingProvider {
    fn estimate_cost(&self, _: &str, _: i64, _: i64, _: i64, _: i64) -> f64 { 0.0 }
    fn estimate_cache_savings(&self, _: &str, _: i64) -> f64 { 0.0 }
    fn cost_from_model_usage(&self, _: Option<&serde_json::Value>, _: Option<&str>, _: i64, _: i64, _: i64, _: i64) -> f64 { 0.0 }
}
...
impl ComplianceProvider for CommunityComplianceProvider {
    fn is_available(&self) -> bool { false }
    fn available_modes(&self) -> Vec<String> { vec![] }
}
...
impl PermissionsProvider for CommunityPermissionsProvider {
    ...
    fn role_permissions(&self, role: &str) -> std::collections::HashSet<crate::permissions::Permission> {
        use crate::permissions::Permission::*;
        match role {
            "owner" | "admin" => std::collections::HashSet::from([
                TracePush, TraceViewAll, TraceViewOwn, PolicyManage,
                CodeBrowse, UserManage, OrgSettingsManage,
            ]),
            "developer" => std::collections::HashSet::from([
                TracePush, TraceViewOwn, CodeBrowse,
            ]),
    ...
    fn is_valid_role(&self, role: &str) -> bool {
        matches!(role, "admin" | "developer" | "owner" | "policy_admin" | "auditor")
    }
}
...
impl SigningProvider for FullSigningProvider {
    fn record_hash(&self, canonical_json: &[u8]) -> String { self.inner.record_hash(canonical_json) }
    fn chain_hash(&self, prev: Option<&str>, record_hash: &str) -> String { self.inner.chain_hash(prev, record_hash) }
    fn sign(&self, record_hash: &str) -> String { self.inner.sign(record_hash) }
    fn verify(&self, record_hash: &str, signature_b64: &str) -> bool { self.inner.verify(record_hash, signature_b64) }
...
impl LlmStoryProvider {
    pub fn new(llm: Arc<dyn StoryLlm>) -> Self { Self { llm } }
}
...
impl StoryProvider for LlmStoryProvider {
    ...
    fn provider_name(&self) -> &str { self.llm.provider_name() }
    fn model_name(&self) -> &str { self.llm.model_name() }
}

pub struct FullPricingProvider;

impl PricingProvider for FullPricingProvider {
    fn estimate_cost(&self, model: &str, input_tokens: i64, output_tokens: i64, cache_read_tokens: i64, cache_write_tokens: i64) -> f64 {
        crate::pricing::estimate_cost(model, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens)
    }
    ...
    fn cost_from_model_usage(&self, model_usage: Option<&serde_json::Value>, fallback_model: Option<&str>, fallback_input: i64, fallback_output: i64, fallback_cache_read: i64, fallback_cache_write: i64) -> f64 {
        crate::pricing::cost_from_model_usage(model_usage, fallback_model, fallback_input, fallback_output, fallback_cache_read, fallback_cache_write)
    }
}
